...
mCube::mCube(std::string textureFilePath, bool flip, bool hasAlpha) {
	shader = new Shader("Shaders/shader.vs", "Shaders/shader.fs");
	texture = new Texture(textureFilePath, flip, hasAlpha);
}

unsigned int mCube::getShaderID() { return shader->ID; }

void mCube::configure() {
	...
	glBufferData(GL_ARRAY_BUFFER, modifiedCubeVertexData.size() * sizeof(CubeVertex), &modifiedCubeVertexData[0], GL_STATIC_DRAW);
...
void mCube::render() {
	...
	glBindVertexArray(VAO);
	glDrawArrays(GL_TRIANGLES, 0, modifiedCubeVertexData.size());
	glBindVertexArray(0);
}
